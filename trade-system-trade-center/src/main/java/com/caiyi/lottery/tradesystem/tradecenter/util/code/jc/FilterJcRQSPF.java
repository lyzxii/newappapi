...
public class FilterJcRQSPF extends FilterBase{

	public static final HashMap<String, String> rqspfMaps = new HashMap<String, String>();
	...
	public void filter(CodeBean bean, FilterResult result) throws CodeFormatException {
		if(bean.getItemType() == CodeBean.NOITEM){	//文件不含场次(需指定场次)
			...
			pattern = Pattern.compile("\\s*RQSPF\\s*\\|(.+)\\|((\\s*\\d+\\s*\\*\\s*\\d+\\s*,\\s*)*(\\s*\\d+\\s*\\*\\s*\\d+\\s*))");
	 ...
	 * RQSPF|110518003=0,110518006=0|2*1_1
	 * @param bean
	 * @param result
	 * @param code
	 * @param codeMaps
...
//		ylist.add("[让球胜平负]110524005=0/3,110524006=3/1|1*1");
		ylist.add("rqspf | 110518003=0,110518006=0,110518007=0 | 2*1 _ 1000 ");
...
//		ylist.add("9→(1,0),10→(1,0),11→(1,0),13→(3,1),16→1,18→(3,1)	64	128	6_1");
		...
		try {
			FilterJcRQSPF rqspf = new FilterJcRQSPF();
			CodeBean bean = new CodeBean();
			bean.setLottype(90);
			bean.setPlaytype("RQSPF");
