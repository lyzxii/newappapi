...
public class FilterSXP extends FilterBase{
	...
	public void filter(CodeBean bean, FilterResult result) throws CodeFormatException {
		if(bean.getItemType() == CodeBean.NOITEM){	//文件不含场次(需指定场次)
			...
			String tmpcode = bean.getCode().replaceAll("上单", "1-1");
			tmpcode = tmpcode.replaceAll("上双", "1-0");
			tmpcode = tmpcode.replaceAll("下单", "0-1");
			tmpcode = tmpcode.replaceAll("下双", "0-0");
...
//		String gg = len + "*1";
	...
	public static void main(String[] args) {
		...
		ylist.add("[上下单双]1=1-1,2=1-1,3=1-1/0-0,4=1-1/0-0,5=1-1|5*1");
		ylist.add("10→上单,11→下双,12→下单,13→下单,14→下双,15→上单	1	2	6_1");
		ylist.add("6→下双,8→下双,19→(上单,上双),29→(上单,上双)	4	8	4_1");
