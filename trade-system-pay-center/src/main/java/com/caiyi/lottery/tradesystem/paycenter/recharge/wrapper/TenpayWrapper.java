...
@Slf4j
@Component("TenpayWrapper")
public class TenpayWrapper {
	@Autowired
	...
	public RechDto addmoney(PayBean bean, ITenpayRech tenpayRech){
		...
		userRechRecordMap.put(bean.getUid()+"_tenpay", rechargeCache);
		cacheBean.setKey(bean.getUid()+"_RechRecordMap");
		cacheBean.setValue(JSONObject.toJSONString(rechargeCache));
		cacheBean.setTime(10*Constants.TIME_MINUTE);
		redisClient.setString(cacheBean, log, SysCodeConstant.PAYCENTER);
		return tenpayRech.addmoney(bean);
	}

	public void backNotify(PayBean bean, ITenpayRech tenpayRech){
		tenpayRech.backNotify(bean);
