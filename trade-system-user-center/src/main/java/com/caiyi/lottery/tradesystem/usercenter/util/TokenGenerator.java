    ...
    public static String[] authToken(String authentication, String appid) {
        ...
        try {
            ...
            if (textValue.length == 2) {
                logger.info("textValue[0]="+textValue[0]);
                logger.info("textValue[1]="+textValue[1]);
                // 拆分值并存入Map中
                HashMap<String, String> map = new HashMap<String, String>();
                String[] tokenValue = textValue[0].split(";");
                for (int i = 0; i < tokenValue.length; i++) {
                    String[] value = tokenValue[i].split("=");
                    logger.info("map.put "+tokenValue[i]+" value.length="+value.length+" key=" + value[0] + " value=" + value[1]);
