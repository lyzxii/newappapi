...
public class StringUtil {
	...
	public static String exchangeString(String attrString){
		if(StringUtil.isEmpty(attrString)){
			return "";
		}
		Integer length = attrString.length();
		char[] attrArray = attrString.toCharArray();
		int arrayLength = 0;
		if(length%2==0){
			arrayLength = length;
		}else{
		...
		for(int i=0;i<arrayLength;i=i+2){
			char temp = attrArray[i];
			attrArray[i]=attrArray[i+1];
			attrArray[i+1] = temp;

		}
		return new String(attrArray, 0, attrArray.length);
	}
	...
	public static boolean checkExt(String ccodes){
		String[] codes=ccodes.split("\\.");
		if(codes.length==2){
			if("txt".equals(codes[1])){
				return true;
			}
		}
		return false;
